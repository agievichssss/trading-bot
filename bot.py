...
# ============= ВСТАВЬ СВОИ НОВЫЕ КЛЮЧИ =============
API_KEY = "CiEfuxenwEgXPwWQYsDdu09fT2uvXGnq7iH5zZiZvuVKOVa23Im6PJFldJMoOWOQMcDdz8Z6xytOSMaNritQ"  # ← ВСТАВЬ НОВЫЙ API KEY
API_SECRET = "pxYOrOORA413kWJ4u9zdanJZcLdg4hxJYXpWwORIdRhHtfqb2ok7CtNsB58jgnTDeHfsiMERl7KtLQtQg"  # ← ВСТАВЬ НОВЫЙ SECRET KEY
...
def get_h1_trend():
    df = get_candles(TIMEFRAME_HIGHER, 100)
    if df is None:
        return None
    slow = sma_shifted(df, 20, 5)
    df = df.dropna()
...
🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"""
                    ...
                    send_telegram(msg)
                    print(f"✅ {signal.upper()} | Цена={price:.0f} | H1={h1_trend}")
                    last_signal = signal
                else:
                    print(f"⚠️ {signal} заблокирован: H1={h1_trend}")
